 ...
 * transform only — never to text — so it stays subtle on scroll. Each blob's
 * own `d` path additionally drifts via CSS keyframes for an idle morph.
 */
export function BlobBackground({ className = "", variant = "section" }: BlobBackgroundProps) {
  ...
  return (
    ...
    >
      <svg
        className={`animate-blob-drift absolute -right-1/4 top-0 ${size} opacity-80`}
        viewBox="0 0 600 600"
      ...
      >
        <defs>
          <linearGradient id="blob-red" x1="0%" y1="0%" x2="100%" y2="100%">
            ...
            <stop offset="100%" stopColor="var(--red-glow)" />
          </linearGradient>
        </defs>
        <path
          className="animate-blob-morph-a"
          fill="url(#blob-red)"
          fillOpacity="0.16"
      ...
      <svg
        className="animate-blob-drift absolute -left-1/3 bottom-0 h-[50vh] w-[50vh] opacity-70 [animation-delay:-6s]"
        viewBox="0 0 600 600"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          className="animate-blob-morph-b"
          fill="none"
          stroke="var(--red)"
          strokeOpacity="0.25"
          strokeWidth="1.5"
